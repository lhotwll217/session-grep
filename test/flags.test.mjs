...
test('--help exits 0 and describes every flag the parser accepts', () => {
  ...
  for (const flag of ['--query', '--any', '--candidates', '--regex', '--session', '--at', '--focus', '--overview', '--skim',
    ...
    assert.match(res.stdout, new RegExp(`^  ${flag}( |$)`, 'm'), `${flag} missing from --help`);
  }
...
test('header reports files_with_matches; JSON keeps the prefilter count beside it', { skip }, () => {
  ...
  assert.equal(out.rawFilesWithHits, 2);
});
...
test('--since skips files last written before the window without reading them', { skip }, () => {
  ...
  assert.equal(out.rawFilesWithHits, 1);
